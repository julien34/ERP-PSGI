...
{	
	...
	/**
	 * Rend un champ null �gal a un string vide
	 * @param s
	 * @return
	 */
	...
	{	
		...
		if(client.getCategorie() == "1"){
			chCategorie.select("Particulier");
		}
		else if(client.getCategorie() == "2"){
			chCategorie.select("Entreprise");
		}
	}
	...
	public void initElements()
	{
		...
		modifier.addActionListener(new ActionListener()
		{
			public void actionPerformed(ActionEvent e)
			{
				if(chCategorie.getSelectedItem() == "Particulier"){
					Categorie.setText("1");
				}
				else if(chCategorie.getSelectedItem() == "Entreprise"){
					Categorie.setText("2");
				}
				...
				if(DatabaseConnection.requete("UPDATE vente_clients SET NOMCLIENT ='"+Nom.getText()+"', PRENOMCLIENT='"+Prenom.getText()+"', ADRESSECLIENT='"+Adresse.getText()+"', EMAILCLIENT='"+Email.getText()+"', TELCLIENT='"+Telephone.getText()+"', CODECATEGORIECLIENT='"+Integer.parseInt(Categorie.getText())+"' WHERE IDCLIENT='"+codeClient+"'"))
				{
					...
					FenetrePrincipale.getPanelClient().raffraichirLigne(idClient, Nom.getText(),Prenom.getText(),Adresse.getText(),Email.getText(),Telephone.getText(), chCategorie.getSelectedItem());
					JOptionPane.showMessageDialog(null, "Client modifi� avec succ�s.", "Modification de Client", JOptionPane.INFORMATION_MESSAGE);
					setVisible(false);
				}
				else JOptionPane.showMessageDialog(null, "Erreur de modification du client. V�rifiez vos champs.", "Modification de client", JOptionPane.WARNING_MESSAGE);
			}
		});
	}
}