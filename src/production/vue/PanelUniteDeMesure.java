...
/**
 * Panel de la gestion d'unite de mesure.
 *
 */
@SuppressWarnings("serial")
public class PanelUniteDeMesure extends JPanel
{
	/**
	...
	/**
	 * Bouton d'ajout d'unité de mesure.
	 */
	private JButton ajouter = new JButton("Ajouter");
	
	/**
	 * Bouton de modification de d'unité de mesure.
	 */
	private JButton modifier = new JButton("Modifier");
	
	/**
	 * Bouton de suppression de d'unité de mesure.
	 */
	private JButton supprimer = new JButton("Supprimer");	
	
	/**
	 * Le tableau qui contient les informations des unités de mesure.
	 */
    ...
    /**
     * Le panel d'ajout d'unité de mesure.
     */
    private PanelAjouterUniteDeMesure panel_ajouter;
    
    /**
     * Le panel de modification d'unité de mesure.
     */
    private PanelModifierUniteDeMesure panel_modifier;
    
    /**
     * Constructeur du panel contenant l'interface de gestion d'unité de mesure.
     */
	public PanelUniteDeMesure()
	{
	...
	public void creationListener()
	{
		...
		ajouter.addActionListener(new ActionListener()
		{
			public void actionPerformed(ActionEvent e)
			{ 
				panel_ajouter = new PanelAjouterUniteDeMesure(PanelUniteDeMesure.this);
		...
		modifier.addActionListener(new ActionListener()
		{
			public void actionPerformed(ActionEvent e)
			{ 
				panel_modifier = new PanelModifierUniteDeMesure(PanelUniteDeMesure.this
		...
		supprimer.addActionListener(new ActionListener()
		{
			public void actionPerformed(ActionEvent e)
			{
				int option = JOptionPane.showConfirmDialog(null, "Confirmer la suppression de l'unite de mesure ?", "Suppression d'unite de mesure", JOptionPane.YES_NO_OPTION);
				if(option == JOptionPane.YES_OPTION)
				{
					...
					if (ligne_choisie != -1)
					{
						try
						{
							...
							ArrayList<UniteDeMesure> liste_unite_de_mesure = DatabaseProduction.getListeUdm();
							int index = 0;

							DatabaseProduction.getRsUdm().beforeFirst();;
							while (DatabaseProduction.getRsUdm().next())
							{
								if (DatabaseProduction.getRsUdm().getInt("code") == code)
								{									
									DatabaseProduction.getRsUdm().deleteRow();
									liste_unite_de_mesure.remove(index);
									raffraichirTable();
								}
								index++;
							}
	...
	public void raffraichirTable()
	{
		...
		ArrayList<UniteDeMesure> unite_de_mesure = DatabaseProduction.getListeUdm();
		Object[][] data = new Object[unite_de_mesure.size()][2];
		int index = 0;

		//Passage d'array list en array. Les attributs des objets sont récupérés
		for (UniteDeMesure object : unite_de_mesure) 
		{
