...
/**
 * Panel de la gestion de produits.
 *
 */
...
public class PanelProduit extends JPanel
{
	 ...
	 */
	private final String[] nom_colonne = {"Code","Nom","Categorie","Prix d'achat","Prix de vente","UDM","Disponibilité"};
    ...
    /**
     * Le panel d'ajout de produit.
     */
    private PanelAjouterProduit panel_ajouter;
    
    /**
     * Le panel de modification de produit.
     */
    private PanelModifierProduit panel_modifier;	
    
    /**
     * Constructeur du panel contenant l'interface de gestion de produits.
     */
	public PanelProduit()
	{
		...
		String[] filtre_colonne_nom = {"Nom","Categorie","Prix d'achat","Prix de vente","UDM","Disponibilité"};
		String[] filtre_operateur_declencheur = {"Prix d'achat","Prix de vente"};
	...
	public void creationListener()
	{
		...
		ajouter.addActionListener(new ActionListener()
		{
			public void actionPerformed(ActionEvent e)
			{ 
				panel_ajouter = new PanelAjouterProduit(PanelProduit.this);
		...
		modifier.addActionListener(new ActionListener()
		{
			public void actionPerformed(ActionEvent e)
			{ 
				panel_modifier = new PanelModifierProduit(PanelProduit.this
				...
				if(option == JOptionPane.YES_OPTION)
				{
					...
					if (ligne_choisie != -1)
					{
						try
						{
							...
							ArrayList<Produit> liste_produit = DatabaseProduction.getListeProduit();
							int index = 0;

							DatabaseProduction.getRsProduit().beforeFirst();
							while (DatabaseProduction.getRsProduit().next())
							{
								if (DatabaseProduction.getRsProduit().getInt("code") == code)
								{									
									DatabaseProduction.getRsProduit().deleteRow();
									liste_produit.remove(index);
									raffraichirTable();
								}
								index++;
							}
	...
	public void raffraichirTable()
	{
		...
		Object[][] data = new Object[produits.size()][7];
		int index = 0;
		...
		for (Produit object : produits) 
		{
			...
			data[index][2] = object.getCategorie().getNom();
			data[index][3] = object.getPrixAchat();
			data[index][4] = object.getPrixVente();
			data[index][5] = object.getUniteDeMesure().getNom();
			data[index][6] = object.getDisponibilite();
			index++;
		}
